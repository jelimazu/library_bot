...
async def statistic(call: CallbackQuery):
    ...
    month = 0
    all_time = 0
    today_ = datetime.datetime.strptime(db.get_now_date(), "%d.%m.%Y")
    ...
    for date in dates:
        if datetime.datetime.strptime(date[0], "%d.%m.%Y") == datetime.datetime.strptime(db.get_now_date(), "%d.%m.%Y"):
            today += 1
        if (datetime.datetime.strptime(date[0], "%d.%m.%Y") == datetime.datetime.strptime(db.get_now_date(), "%d.%m.%Y")
...
async def send_mailing(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await call.message.answer("Введите текст или фото рассылки",
...
async def set_message(message: Message, state: FSMContext):
    if message.photo:
        ...
        await state.update_data(message=message.html_text, type="text")
...
async def set_count(message: Message, state: FSMContext):
    ...
    if count.isdigit():
        if int(count) == 0:
            await message.answer("Вы уверены что хотите отправить всем пользователям?",
                                 reply_markup=ask_mailing_keyboard())
            await state.update_data(count=-1)
        elif int(count) > 0:
            await message.answer("Вы уверены что хотите отправить рассылку?",
                                 reply_markup=ask_mailing_keyboard())
...
async def start_send_mailing(call: CallbackQuery, state: FSMContext):
    ...
    for user in users:
        try:
            if type_ == "text":
                await call.message.bot.send_message(user[0], message)
            elif type_ == "photo":
                await call.message.bot.send_photo(user[0], photo=photo, caption=message)
